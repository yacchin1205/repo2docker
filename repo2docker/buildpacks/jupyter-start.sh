...
# from https://github.com/jupyter/docker-stacks/blob/master/base-notebook/start.sh
...
if [ "$(id -u)" == 0 ] ; then
    ...
    sed -r "s#Defaults\s+secure_path\s*=\s*\"?([^\"]+)\"?#Defaults secure_path=\"\1:${CONDA_DIR}/bin:${PATH}\"#" /etc/sudoers | grep secure_path > /etc/sudoers.d/path
